////////////////////////GENEROWANIE GALERII PRZEPISÓW////////////////////////////////
document.addEventListener("DOMContentLoaded", () => 
{
    fetch('bozenarodz/bozenarodz.json') // Wczytaj dane JSON
        .then(response => response.json())
        .then(data => {
            const gallery = document.getElementById('gallery');
            data.forEach(recipe => {
                // Tworzy elementy galerii
                const item = document.createElement('div');
                item.className = 'gallery-item';
                item.innerHTML = `
                    <img src="bozenarodz/${recipe.image}" alt="${recipe.title}">
                    <p>${recipe.title}</p>
                `;
...
function openRecipe(id) {
    window.location.href = `przepis.html?id=${id}&category=bozenarodz`;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Wczytaj dane JSON
    fetch('bozenarodz/bozenarodz.json')
        .then(response => response.json())
        .then(data => {
