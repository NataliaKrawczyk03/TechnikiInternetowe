document.addEventListener("DOMContentLoaded", () => {
    fetch('salatki/salatki.json') // Wczytaj dane JSON
        ...
        .then(data => {
            ...
            data.forEach(recipe => {
                ...
                item.innerHTML = `
                    <img src="salatki/${recipe.image}" alt="${recipe.title}">
                    <p>${recipe.title}</p>
                `;
...
function openRecipe(id) {
    window.location.href = `przepis.html?id=${id}&category=salatki`;
}
